...
import { useRef, useEffect, type SVGProps } from 'react'
...
export const TodoList = ({ statuses }: Props) => {
  ...
  const { mutate: updateTodoStatus, isLoading: isChecking } =
        ...
        <li key={todo.id}>
          <div
            className={`flex items-center rounded-12 border border-gray-200 px-4 py-3 shadow-sm ${
              todo.status === 'completed' ? 'bg-gray-200 text-gray-500' : ''
            }`}
          >
            <Checkbox.Root
              id={String(todo.id)}
